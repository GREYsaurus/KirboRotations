...
internal class MCH_Kirbo_AIO : MCH_Base
{
    ...
    protected override IRotationConfigSet CreateConfiguration()
    {
        ...
        .SetCombo("OpenerSelection", 5, "Opener Selection", "Early AA", "Delayed Tools", "123 Tools", "Fast Wildfire", "The FFlogs Opener", "No Opener please");
    }
    ...
    {
        var DoingRaids = TerritoryContentType is TerritoryContentType.Raids;
        var DoingUltimateRaids = TerritoryContentType is TerritoryContentType.UltimateRaids;
        var RaidingORUltimate = DoingRaids || DoingUltimateRaids;
    ...
    public override void DisplayStatus()
    {
        ...
        if (ImGui.CollapsingHeader("| Other Info |"))
        {
            if (ImGui.BeginTable("OtherInfo", 2, ImGuiTableFlags.Borders | ImGuiTableFlags.BordersInner | ImGuiTableFlags.Resizable))
            {
                ImGui.Indent();
                ImGui.TableNextColumn(); ImGui.TextColored(Purple, $"Ping:    {Ping}ms ");
                ImGui.TableNextColumn(); ImGui.TextColored(yellow, "GCD time remaining: " + WeaponRemain);
                ImGui.TableNextRow();
                ImGui.TableNextColumn(); ImGui.TextColored(yellow, "GCD time: " + WeaponTotal);
                ImGui.TableNextColumn(); ImGui.TextColored(yellow, "GCD window: " + WeaponElapsed);
                ImGui.TableNextRow();
                ImGui.TableNextColumn(); ImGui.TextColored(yellow, "oGCD time left: " + NextAbilityToNextGCD);
                ImGui.TableNextColumn(); //ImGui.Text(InHighEndDuty.ToString());
                ImGui.TableNextRow();
...
internal abstract partial class MCH_2 : MCH_Kirbo_AIO
{
    #region Rotation Info
    public override string GameVersion => "?.??";
    public override string RotationName => "Kirbo's - All In One";
    public override string Description => "Will this even Work?";
    #endregion Rotation Info

    public void GCD(out IAction act)
    {
        throw new System.NotImplementedException();
    }
    public void oGCD(out IAction act)
    {
        throw new System.NotImplementedException();
    }
}